...
export default function ProviderCard({ provider, onBook }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-outline-variant/10 shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 overflow-hidden group">
      {/* Header */}
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-on-surface-variant text-xs flex items-center gap-1 mt-0.5">
                  ...
                  {provider.servicesOffered && provider.servicesOffered.length > 0 ? provider.servicesOffered[0].category : provider.service}
                  {provider.verified && (
                    <span className="material-symbols-outlined text-tertiary text-xs fill-icon ml-1" title="Verified">verified</span>
                  )}
                </p>
              </div>
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-all hover:scale-110 ${
                  isWishlisted ? "bg-secondary/10 text-secondary" : "bg-surface-container text-on-surface-variant hover:text-secondary"
                }`}
              >
      ...
      <div className="px-5 pb-4 flex items-center justify-between text-sm">
        ...
        <div className="text-right">
          ...
          <p className="font-extrabold text-primary text-base">₹{provider.servicesOffered && provider.servicesOffered.length > 0 ? Math.min(...provider.servicesOffered.map(s => s.price)) : provider.price}</p>
        </div>
      </div>
      ...
      <div className="px-5 pb-5 flex gap-2 border-t border-outline-variant/10 pt-3">
        ...
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all hover:scale-[0.98] ${
            provider.available
