...
export default function FilterSidebar({ filters, onChange, onSortChange, sortBy, onDetectLocation, userLocation }) {
  const [expanded, setExpanded] = useState({ sort: true, rating: true, price: true, distance: true, experience: true, availability: true });
    ...
    <div className="w-full space-y-1">
      <div className="bg-white rounded-2xl border border-outline-variant/10 overflow-hidden shadow-sm">
        ...
        <div className="px-5 py-4 border-b border-outline-variant/10 flex items-center justify-between">
          ...
          <button
            onClick={() => onChange({ rating: 0, minPrice: 0, maxPrice: 5000, distance: 20, availability: "all", verifiedOnly: false, minExperience: 0 })}
          ...
          <div className="space-y-1">
            {SORT_OPTIONS.map(opt => (
              <label key={opt.value} className="flex items-center gap-3 p-2 rounded-xl hover:bg-surface-container-low cursor-pointer">
                <input type="radio" name="sort" value={opt.value} checked={sortBy === opt.value} onChange={() => onSortChange(opt.value)} className="accent-primary" />
                ...
                {opt.value === "ai" && <span className="ml-auto text-[9px] font-black bg-secondary/20 text-secondary px-1.5 py-0.5 rounded-full">SMART</span>}
              </label>
            ))}
          </div>
        </Section>
          ...
          <div className="space-y-1">
            {[0, 3, 4, 4.5].map(r => (
              <label key={r} className="flex items-center gap-2 p-2 rounded-xl hover:bg-surface-container-low cursor-pointer">
                <input type="radio" name="rating" checked={filters.rating === r} onChange={() => setFilter("rating", r)} className="accent-primary" />
        ...
        <Section label="Price Range" icon="payments" expanded={expanded.price} onToggle={() => toggle("price")}>
          <div className="px-2 space-y-3">
            ...
            <input type="range" min="0" max="5000" step="100" value={filters.maxPrice}
              onChange={e => setFilter("maxPrice", Number(e.target.value))} className="w-full accent-primary" />
            <div className="flex gap-2">
              {[500, 1000, 2000, 5000].map(p => (
                <button key={p} onClick={() => setFilter("maxPrice", p)}
                  className={`flex-1 py-1 text-xs font-semibold rounded-lg border transition-all ${filters.maxPrice === p ? "border-primary bg-primary/10 text-primary" : "border-outline-variant/30 text-on-surface-variant hover:border-primary/40"}`}>
                  ₹{p <= 999 ? p : p / 1000 + "k"}
                </button>
              ))}
            </div>
        ...
        <Section label="Distance" icon="near_me" expanded={expanded.distance} onToggle={() => toggle("distance")}>
          <div className="px-2 space-y-2">
            {/* GPS button */}
            <button onClick={onDetectLocation}
              className="w-full flex items-center gap-2 text-xs font-semibold text-primary bg-primary/5 border border-primary/20 px-3 py-2 rounded-xl hover:bg-primary/10 transition-colors">
              <span className="material-symbols-outlined text-sm">my_location</span>
              {userLocation ? userLocation.label : "Use My GPS Location"}
            </button>
            <p className="text-xs text-on-surface-variant">Within <span className="font-bold text-primary">{filters.distance} km</span></p>
            <input type="range" min="1" max="20" value={filters.distance}
              onChange={e => setFilter("distance", Number(e.target.value))} className="w-full accent-primary" />
          </div>
        </Section>

        {/* Experience */}
        <Section label="Experience" icon="work_history" expanded={expanded.experience} onToggle={() => toggle("experience")}>
          <div className="flex flex-wrap gap-2">
            {EXP_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setFilter("minExperience", opt.value)}
                className={`py-1.5 px-3 text-xs font-semibold rounded-xl border transition-all ${filters.minExperience === opt.value ? "border-primary bg-primary/10 text-primary" : "border-outline-variant/30 text-on-surface-variant hover:border-primary/40"}`}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        <Section label="Availability" icon="calendar_month" expanded={expanded.availability} onToggle={() => toggle("availability")}>
          <div className="grid grid-cols-3 gap-2">
            {["all", "today", "tomorrow"].map(a => (
              <button key={a} onClick={() => setFilter("availability", a)}
                className={`py-2 text-xs font-semibold rounded-xl border transition-all capitalize ${filters.availability === a ? "border-primary bg-primary/10 text-primary" : "border-outline-variant/30 text-on-surface-variant hover:border-primary/40"}`}>
        ...
        <div className="px-5 py-4 border-t border-outline-variant/10">
          <label className="flex items-center justify-between cursor-pointer">
            ...
            <div onClick={() => setFilter("verifiedOnly", !filters.verifiedOnly)}
